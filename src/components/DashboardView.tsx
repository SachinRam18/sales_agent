...
export default function DashboardView({ onNavigateTo, onSelectCompany, userRole }: DashboardViewProps) {
  ...
  if (loading) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[60vh] gap-3 text-slate-500 font-sans">
  ...
  if (error || !data) {
    return (
      <div className="p-8 max-w-xl mx-auto text-center font-sans">
        ...
        <p className="font-semibold text-slate-900 text-sm">Dashboard Load Error</p>
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 font-sans" id="dashboard-view-root">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        ...
        <div className="flex items-center gap-2.5">
          <span className="text-[11px] font-mono text-slate-500 bg-slate-100 px-2.5 py-1 rounded border border-slate-200 font-medium">
            Role: <strong className="text-slate-900 font-semibold">{userRole}</strong>
          </span>
          <button 
            onClick={fetchDashboard}
            className="p-1.5 hover:bg-slate-50 text-slate-500 rounded-lg border border-slate-200 hover:text-slate-800 transition shadow-sm"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4" id="dashboard-metrics-grid">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <span className="text-[10px] uppercase font-semibold tracking-wider text-slate-500 block">Total Pipeline</span>
          <div className="mt-2 flex items-baseline justify-between animate-fade-in">
            <span className="text-xl font-semibold text-slate-900">{metrics.totalLeads}</span>
            <Users className="w-4 h-4 text-slate-400" />
          </div>
          <span className="text-[9px] text-slate-400 mt-1 block">In-Memory Entities</span>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <span className="text-[10px] uppercase font-semibold tracking-wider text-slate-500 block">ICP Qualified</span>
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-xl font-semibold text-slate-900">{metrics.qualifiedLeads}</span>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <span className="text-[10px] uppercase font-semibold tracking-wider text-slate-500 block">Outreach Sent</span>
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-xl font-semibold text-slate-900">{metrics.outreachSent}</span>
            <Send className="w-4 h-4 text-slate-600" />
          </div>
          <span className="text-[9px] text-slate-400 mt-1 block">Email & LinkedIn</span>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <span className="text-[10px] uppercase font-semibold tracking-wider text-slate-500 block">Active Campaigns</span>
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-xl font-semibold text-slate-900">{metrics.activeCampaigns}</span>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <span className="text-[10px] uppercase font-semibold tracking-wider text-slate-500 block">Conversion Rate</span>
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-xl font-semibold text-slate-900">{metrics.conversionRate}%</span>
            <Layers className="w-4 h-4 text-slate-500" />
          </div>
          <span className="text-[9px] text-slate-400 mt-1 block">Meeting Sched %</span>
        </div>

        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <span className="text-[10px] uppercase font-semibold tracking-wider text-slate-500 block">CRM Records</span>
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-xl font-semibold text-slate-900">{metrics.crmRecords}</span>
            <BadgeCheck className="w-4 h-4 text-slate-600" />
          </div>
          <span className="text-[9px] text-slate-600 font-medium mt-1 block">No Duplicates</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" id="dashboard-grid-content">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="font-semibold text-slate-900 text-sm">Lead Funnel Distribution</h3>
                <p className="text-[11px] text-slate-400">Pipeline states mapped from database store files</p>
              </div>
              <span className="text-xs font-semibold text-slate-600 bg-slate-50 px-2.5 py-1 rounded border border-slate-200">
                Active Stages
              </span>
            </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h3 className="font-semibold text-slate-900 text-sm">Top Rated Qualified Prospects</h3>
                <p className="text-[11px] text-slate-400">Prioritized lead lists derived from custom AI grades</p>
              </div>
              <button 
                onClick={() => onNavigateTo("crm")}
                className="text-xs text-slate-700 font-semibold hover:text-slate-900 hover:underline"
              >
                View Pipeline &rarr;
              </button>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {topLeads.map((company) => (
                    <tr 
                      key={company.id} 
                      className="hover:bg-slate-50/75 cursor-pointer group"
                      onClick={() => onSelectCompany(company.id)}
                    >
                      <td className="py-3 font-semibold text-slate-900 pr-2 block truncate max-w-[170px]">
                        {company.name}
                        <span className="text-[10px] text-slate-400 block font-normal">{company.location}</span>
                      </td>
                      <td className="py-3 text-slate-500 pr-2">{company.industry}</td>
                      <td className="py-3 text-center pr-2">
                        <span className={`px-2 py-0.5 rounded-full font-bold text-[10px] ${company.score >= 90 ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-800"}`}>
                          {company.score}
                        </span>
                      </td>
                      <td className="py-3 text-center pr-2">
                      ...
                      <td className="py-3 text-right">
                        <button className="text-slate-900 hover:text-slate-700 font-semibold text-[11px] group-hover:underline">
                          Analyze
                        </button>
                      </td>
                    </tr>
                  ...
                  {topLeads.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-4 text-center text-slate-400 text-xs">No CRM entries created yet. Launch Lead Discovery to populate.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-slate-900 border border-slate-800 text-white p-6 rounded-2xl shadow-sm relative overflow-hidden">
            ...
            <p className="text-[10px] text-slate-400 mb-4 font-mono">Autonomous suggestions for qualification optimization</p>

            <div className="space-y-4">
              {aiRecommendations.map((rec) => (
                <div key={rec.id} className="bg-white/5 p-3 rounded-lg border border-white/10 space-y-1">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-semibold text-slate-900 text-sm">SDR Activity Logs</h3>
                <p className="text-[11px] text-slate-400">Verbatim updates from active agents</p>
              </div>
            ...
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
              {recentActivities.map((act) => (
                <div key={act.id} className="text-xs flex gap-2.5 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5 font-normal">{act.description}</p>
              ...
              {recentActivities.length === 0 && (
                <p className="text-center text-slate-400 py-4 font-mono">No recent SDR updates dispatched.</p>
              )}
            </div>
          </div>
