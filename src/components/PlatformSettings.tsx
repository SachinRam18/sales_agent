...
export default function PlatformSettings({ userRole, onChangeRole, userEmail }: PlatformSettingsProps) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 font-sans" id="settings-root">
      ...
      {testLog && (
        <div className="p-3 bg-emerald-50 text-emerald-800 border border-emerald-100 rounded-xl text-xs font-mono font-bold animate-fade-in flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" id="settings-grid-wrapper">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-slate-200/80 rounded-2xl p-5 space-y-4 shadow-sm">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-slate-50 pb-2 flex items-center gap-1.5">
              ...
              <div className="space-y-2">
                {(["Admin", "Team Member", "Viewer"] as const).map((r) => (
                  <button
                    ...
                    className={`w-full text-left p-3.5 rounded-xl border text-xs transition duration-150 flex items-center justify-between ${userRole === r ? "bg-indigo-50 border-indigo-600 text-indigo-900 font-bold" : "border-slate-100 hover:bg-slate-50 text-slate-600"}`}
                  >
                    <div>
          ...
          <div className="bg-slate-900 text-slate-350 p-5 rounded-2xl border border-slate-800 space-y-3 shadow-md">
            ...
            <div className="space-y-1 text-[11px] font-mono">
              ...
              <div className="pt-2 text-[10px] text-slate-500 font-sans italic">Ready to link to actual PostgreSQL setups via migrations port triggers.</div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200/60 rounded-2xl p-6 shadow-sm space-y-5">
            <div>
              <h3 className="font-bold text-slate-800 text-sm">Synchronized Outbound Integrations</h3>
              <p className="text-xs text-slate-400 mt-0.5">Maintain API tokens for target prospecting systems securely.</p>
            </div>
                  ...
                  <div key={key} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 font-mono">
                    <div className="space-y-1 font-sans">
                      <div className="flex items-center gap-2">
                        <strong className="text-xs text-slate-800 uppercase font-bold tracking-wider">{key} Connector</strong>
                        {item.enabled ? (
                          <span className="text-[9px] bg-emerald-50 text-emerald-800 border border-emerald-100 font-bold px-1.5 py-0.5 rounded-full">
                            OAuth Connection Connected
                          </span>
                        ) : (
                          <span className="text-[9px] bg-slate-50 text-slate-400 border border-slate-100 font-bold px-1.5 py-0.5 rounded-full">
                            Inactive Link
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0 self-start sm:self-center">
                      {item.enabled && (
                        <button
                          ...
                          className="px-2.5 py-1 text-[11px] font-sans font-bold text-slate-600 hover:text-indigo-600 bg-slate-50 border border-slate-200 rounded hover:bg-slate-100"
                        >
                          Send Ping
                        </button>
                      )}
                      <button
                        ...
                        className={`p-1 rounded text-slate-400 transition ${item.enabled ? "text-indigo-600" : "text-slate-350"}`}
