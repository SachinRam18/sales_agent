    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 font-sans" id="lead-discovery-root">
      ...
      <div className="border-b border-slate-200 pb-5">
        <h1 className="text-xl font-semibold text-slate-900 tracking-tight flex items-center gap-2">
          <SlidersHorizontal className="w-4 h-4 text-slate-800" /> Lead Discovery Agent Console
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <form className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-6 space-y-5" onSubmit={runDiscoveryPipeline} id="form-discovery-search">
            <div className="flex items-center justify-between border-b border-slate-50 pb-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Target Settings</h3>
            ...
            {icpProfiles.length > 0 && (
              <div>
                <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block mb-1">Apply ICP Profile Rule Preset</label>
                <select 
                  value={selectedIcpId} 
                  onChange={handleIcpSelectChange}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg py-2 px-3 text-xs focus:outline-none focus:border-slate-900 focus:bg-white"
                >
            ...
            <div className="space-y-4 pt-1">
              <div>
                <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1 font-sans">Corporate Sector / Industry</label>
                <input
                  type="text"
                  value={industry}
              ...
              <div>
                <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1">Geographic Location / Country</label>
                <input
                  type="text"
                  value={country}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1">Size Bracket</label>
                  <input
                    type="text"
                    value={companySize}
                ...
                <div>
                  <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1">Est. Revenue</label>
                  <input
                    type="text"
                    value={revenueRange}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg py-2 px-3 text-xs focus:outline-none focus:border-slate-900 focus:bg-white resize-none"
                />
              </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 space-y-5">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-100 pb-3">Agent Orchestration Monitor</h3>
              ...
              {[
                ...
                return (
                  <div key={s.step} className="space-y-2 relative">
                    ...
                    <div>
                      <p className="text-[10px] font-semibold text-slate-700 truncate">{s.label}</p>
            ...
            {/* Log display */}
            {loading && (
              <div className="bg-slate-950 text-slate-350 font-mono text-[10px] p-4 rounded-xl space-y-1.5 h-32 overflow-y-auto border border-slate-800" id="terminal-discovery">
                <p className="text-slate-400 font-semibold">--- AUTONOMOUS AGENT STREAM INITIATED ---</p>
                ...
                {activeStep! >= 3 && <p className="text-slate-400">&gt; [Enrichment Agent] Scanning company domains to collect installed tech stack and key decision-makers.</p>}
            ...
            {results.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center justify-between text-xs text-slate-500">
                  <span className="font-semibold text-slate-700">DISCOVERED ACTIVE LEADS ({results.length})</span>
                        ...
                        <tr key={lead.id} className="hover:bg-slate-50/50">
                          <td className="py-3.5 pr-2 max-w-[200px]">
                            <div className="font-semibold text-slate-900 flex items-center gap-1.5">
                              {lead.name}
                              <a href={lead.website} target="_blank" rel="noopener noreferrer" className="p-0.5 hover:bg-slate-100 rounded text-slate-400 hover:text-slate-900">
                                <ExternalLink className="w-3 h-3" />
                              </a>
                            </div>
                            <span className="text-[10px] text-slate-400 block font-normal truncate mt-0.5">{lead.website}</span>
                          </td>
                          <td className="py-3.5 pr-2 text-[11px] text-slate-600 space-y-0.5">
                            <div>Location: <strong className="text-slate-800 font-medium">{lead.location}</strong></div>
                            <div>Employees: <strong className="text-slate-800 font-medium">{lead.employees}</strong></div>
                            <div>Tech: <strong className="text-teal-600 text-[10px] font-mono bg-teal-50 px-1 py-0.5 rounded">{lead.technologies}</strong></div>
                          </td>
                          <td className="py-3.5 pr-2 text-[10px] text-slate-500 space-y-1">
                            {lead.contacts && lead.contacts.map((c, ci) => (
                              <div key={ci} className="border-l-2 border-slate-200 pl-1.5">
                                <strong className="text-slate-700 block text-[11px] font-medium">{c.name}</strong>
                                <span className="block truncate max-w-[120px]">{c.role}</span>
                              </div>
                            ))}
                          </td>
                          <td className="py-3.5 pr-2 text-center">
                            <span className={`px-2 py-0.5 rounded font-semibold block text-center max-w-[50px] mx-auto text-[10px] ${lead.score >= 90 ? "bg-emerald-50 text-emerald-800 border border-emerald-100" : "bg-slate-100 text-slate-800 border border-slate-200"}`}>
                              {lead.score}
                            </span>
                          </td>
                          <td className="py-3.5 text-right font-medium">
                            {syncedIds[lead.id] ? (
                              ...
                              <button
                                ...
                                className="bg-white hover:bg-slate-50 text-slate-900 border border-slate-200 font-semibold text-[11px] px-3 py-1.5 rounded-md transition shadow-sm cursor-pointer"
                              >
                                Sync Import
                              </button>
                            )}
